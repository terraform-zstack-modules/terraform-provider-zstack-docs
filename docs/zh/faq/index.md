...
客户文档和公开示例使用 `ZStack-Robot/zstack`。如果是 provider 开发环境，需要调试本地构建或开发 Registry，可以按开发环境配置 source 或 Terraform CLI dev override。离线客户环境应优先使用 Terraform provider mirror 提供同一 provider source 和版本，不要在交付示例里随意替换 source。
...
自动化脚本和 CI/CD 配置优先 UUID。人工编写且名称唯一时可以用 name。`name_pattern` 只适合探索和调试。

## 示例可以直接用于生产吗？

示例是可运行起点，不是完整生产模块。生产使用前需要补充 backend、变量校验、权限、命名规范、标签和审批流程。
...
支持，但建议通过 Terraform provider mirror 提供 `ZStack-Robot/zstack` 的目标版本，并确认 runner 能从内网 mirror 获取 provider。
...
`name_pattern`。资源编排功能及编排模板已取消，不再维护对应示例。

## Load Balancer 示例为什么没有完整后端绑定？

后端成员绑定方式和客户环境、provider 版本、网络模型有关。当前示例先创建 VIP、LB、listener 和 server group，并输出 UUID，便于按目标环境补充后端绑定。

## 脚本初始化适合做什么？

适合 day-1 初始化，例如安装软件、配置 guest agent、写入基础配置。不适合写入长期密钥、私钥或不可审计的大型脚本。
...
## CDP 和 Backup 示例为什么有很多 `replace-me`？

备份/CDP 强依赖真实资源 UUID、backup storage 类型和业务保留策略。文档不能替客户编造这些值，必须由管理员从环境中确认。

## Scheduler 的 job type 应该填什么？

取决于 ZStack 环境支持的任务类型。使用前应查询或确认平台支持的 job type，不要自行猜测。

## Network Observability 示例能直接套用吗？
